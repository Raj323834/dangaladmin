...
@Component({
  templateUrl: 'pokerToDgReport.component.html',
  styleUrls: ['pokerToDgReport.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class PokerToDgTransferReportComponent {
  ...
  columns: string[] = ['userId', 'amount', 'openingDepositBalance','closingDepositBalance', 'openingWithdrawalBalance', 'closingWithdrawalBalance','date'];
  ...
  private title: string = 'PokerToDG Report';
  private excel: any = [];
  ...
  ngOnInit() {
    ...
    const accessPermission = HelperService.getModulePermissions(PermissionNames.POKER_TO_DG_TRANSFER_REPORT);
  ...
  getPokerToDgReportDetails = () => {
    ...
    if (isValid) {
      ...
      this.reportService.getPokerToDgReport(UrlConstant.getPokerToDgdGReportUrl, AppConstants.NO_URL_PARAM, this.searchParamsForm.value, this.ipAddress, userName, '' + Date.now())
        .subscribe(resData => {
          this.excel = [];
          this.allData = [];
